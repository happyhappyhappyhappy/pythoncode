...
for i in range(1,101):
    graph.add('A'+str(i),'B'+str(i),x)
for i in range(1,100):
    graph.add('A'+str(i+1),'B'+str(i),x)
for i in range(1,100):
    graph.add('A'+str(i),'A'+str(i+1),y)
    graph.add('B'+str(i),'B'+str(i+1),y)
d,_ = graph.Dijkstra_search('A'+str(a))#始点から各点までの最短経路と最短経路を求めるのに必要なprevのうち、使用するのは一つ目のみ
print(d['B'+str(b)])
